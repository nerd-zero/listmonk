...
| Auth | **Zitadel (OIDC) — client built, not live-tested** | `internal/authn` verifies the bearer token against Zitadel's JWKS (go-oidc) and JIT-provisions a `users` row keyed by the `sub` claim on first sight (`internal/provisioning.JITProvisionUser`, live-verified for idempotency against real Postgres). The JWKS verification itself isn't live-tested — no Zitadel tenant is configured in dev. |
