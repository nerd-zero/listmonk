...
Status: **built and fully live**. Both repos' work described below is done and live-verified (listmonk fork PR #47, listnun PR #25). Cloudflare's "SSL for SaaS" (Custom Hostnames API) is now enabled on our zone, so `provisioning.Service.AddCustomDomain`/`GetCustomDomain`/`DeleteCustomDomain` call the real Custom Hostnames API again (`internal/cloudflareclient`) instead of the interim plain-DNS-CNAME check a short-lived rollback (listnun PR #27) had substituted while SSL for SaaS was still pending. A custom domain now gets a real, Cloudflare-issued TLS certificate and is served over HTTPS once its Custom Hostname's `ssl.status` flips to `active`.
...
3. **UI shows the DNS records to publish** — a CNAME (`mail.acme.com` → **our own wildcard hostname**, `CLOUDFLARE_FALLBACK_ORIGIN`, e.g. `cname.listnun.app`) and Cloudflare's ownership-verification TXT — using the same record-list-with-copy-buttons component pattern as `SenderIdentityCard`. **The org's own DNS does not need to be on Cloudflare** — Route 53, GoDaddy, Namecheap, anywhere works, since a CNAME record behaves the same regardless of registrar/host. It's *our* hostname that's proxied through our Cloudflare zone, which is what lets Cloudflare terminate TLS and route the request once it arrives — the org is never asked to touch Cloudflare themselves.
